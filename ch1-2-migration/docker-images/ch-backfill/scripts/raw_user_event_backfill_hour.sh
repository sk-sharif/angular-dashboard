...
echo "|--------Back-filling using [ `cat ./sql/raw_user_event_backfill_hour.sql  | sed "s/PROCESS_DATE/$temp_date/g"` ]"
cat ./sql/raw_user_event_backfill_hour.sql  | sed "s/PROCESS_DATE/$temp_date/g"| ssh -o StrictHostKeyChecking=no "${CH_SSH_USER_HOSTNAME1}" -T "cat | clickhouse-client -h \"${CLICKHOUSE_HOSTNAME1}\" -d \"${CLICKHOUSE_DATABASE1}\" --multiline --multiquery"